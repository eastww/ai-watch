...
**现象（实测日志，commit f561966）**：
- WiFi 连接正常、SNTP 启动、时间同步成功 ✅
- 但**触摸屏幕（触发 M3 ASR）立即死机**，无任何 ASR 日志输出

**根因（栈溢出）**：
- `asr_feed()` 里声明 `char frame[ASR_MAX_PCM_BYTES * 3 + 128]`：
  `ASR_MAX_PCM_BYTES = 16000*2*64/1000 = 2048` → `frame` 高达 **6272 字节**，
  且直接在**栈上**。
- 调用链：`asr_feed` ← `m3_record_cb`（录音回调）← `record_task`，
  而 `record_task` 的**栈只有 4096 字节** → 6KB 栈数组瞬间撑爆任务栈 →
  栈溢出 → 内存踩踏 → 死机。
- 此前版本没炸是因为时间未同步，ASR 从未真正走到 `asr_feed`。

**修复**（`main/src/cloud/asr_client.c` + `main/src/audio/audio.c`）：
1. `asr_feed()` 的 `frame` 和 `b64buf` 改为**静态缓冲**（单 ASR 会话安全，
   不占任务栈）。
2. `record_task` 栈从 4096 → **8192** 字节（给回调留足余量）。

**经验教训**：
1. **嵌入式任务栈很宝贵**：回调到底层时，绝不能在大栈局部数组；
   大缓冲一律静态区或堆（`malloc`）。
2. 判断栈溢出的特征：某操作（如触摸触发）后立即无日志死机/复位，
   检查该调用链中所有函数的最大栈占用是否超过任务栈。
3. 起任务时留 2 倍余量比出事后排查成本低得多。
